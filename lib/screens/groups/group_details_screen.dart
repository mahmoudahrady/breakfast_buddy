...
class _GroupDetailsScreenState extends State<GroupDetailsScreen>
  ...
  void _loadGroupDetails() async {
    ...
    // Initialize today's session for this group to enable payment management
  ...
  void _copyPaymentSummary(
    ...
    buffer.writeln('───────────────────────────────');
    buffer.writeln('Total Amount: ${totalAmount.toStringAsFixed(2)} SAR');
    buffer.writeln('Paid: ${totalPaid.toStringAsFixed(2)} SAR ($paidCount members)');
    buffer.writeln('Unpaid: ${totalUnpaid.toStringAsFixed(2)} SAR ($unpaidCount members)');
    buffer.writeln('');

    // Individual Payments
    buffer.writeln('👥 MEMBER PAYMENTS');
    buffer.writeln('───────────────────────────────');

    // Sort: Unpaid first, then paid
    final sortedPayments = List<Map<String, dynamic>>.from(memberPayments);
    sortedPayments.sort((a, b) {
      final paymentA = a['payment'] as Payment?;
      final paymentB = b['payment'] as Payment?;

      // Members with no payment go to the end
      if (paymentA == null && paymentB == null) return 0;
      if (paymentA == null) return 1;
      if (paymentB == null) return -1;

      // Unpaid before paid
      if (!paymentA.paid && paymentB.paid) return -1;
      if (paymentA.paid && !paymentB.paid) return 1;

      // Sort by amount (highest first)
      return paymentB.amount.compareTo(paymentA.amount);
    });

    for (var item in sortedPayments) {
      final member = item['member'] as GroupMember;
      final payment = item['payment'] as Payment?;

      if (payment != null) {
        final status = payment.paid ? '✅ Paid' : '⏳ Pending';
        final amount = payment.amount.toStringAsFixed(2);
        buffer.writeln(member.userName);
        buffer.writeln('  Amount: $amount SAR');
        buffer.writeln('  Status: $status');
        if (payment.paid && payment.paidAt != null) {
          buffer.writeln('  Paid on: ${dateFormat.format(payment.paidAt!)}');
        }
        buffer.writeln('');
      } else {
        buffer.writeln(member.userName);
        buffer.writeln('  Amount: 0.00 SAR');
        buffer.writeln('  Status: ⚪ No orders');
        buffer.writeln('');
      }
    }

    buffer.writeln('═══════════════════════════════');
    buffer.writeln('Generated by Breakfast Buddy');
    buffer.writeln('═══════════════════════════════');
            ...
            onPressed: () async {
              ...
              if (mounted) {
                if (success) {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text(orderProvider.errorMessage ?? 'Failed to mark payment as paid'),
                      backgroundColor: Colors.red,
                    ),
                  );
                }
